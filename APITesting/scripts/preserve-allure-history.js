...
if (!fs.existsSync(reportHistory)) {
  console.log('[allure-history] No previous report found.');
  process.exit(0);
}
...
for (const file of files) {
  fs.copyFileSync(
    path.join(reportHistory, file),
    path.join(resultsHistory, file),
  );
}

console.log(`[allure-history] Copied ${files.length} history file(s).`);